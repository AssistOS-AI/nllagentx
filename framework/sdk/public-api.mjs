...
export const sdkSurfaces = Object.freeze([
  ...
  surface("cnl", "framework/sdk/cnl/index.mjs", "Typed controlled-language frames, canonical rendering, parsing, and semantic comparison.", cnl,
    "import { findingFrame, literalSlot, renderCanonicalCNL } from \"./framework/sdk/cnl/index.mjs\";\nconst text = renderCanonicalCNL(findingFrame(\"CHECK\").set(\"status\", literalSlot(\"UNKNOWN\")).seal());"),
  surface("intent", "framework/sdk/intent/index.mjs", "IntentJS modes, domains, concerns, evidence, assurance, outputs, fallback, and load profiles.", intent,
    "import { intent, analyze, explicitDomain, concern, allCompatible } from \"./framework/sdk/intent/index.mjs\";\nexport default intent(\"task\").mode(analyze()).domains(explicitDomain(\"logic-basic\")).concerns(concern(\"consistency\")).whenUnclear(allCompatible()).seal();"),
