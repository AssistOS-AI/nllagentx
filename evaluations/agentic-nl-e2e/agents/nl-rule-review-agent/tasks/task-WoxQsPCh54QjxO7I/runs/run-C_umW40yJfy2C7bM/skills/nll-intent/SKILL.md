...
In adaptive mode, name the semantic operation as a capability-bearing concern that the circuit registry can prove or expose as missing. Distinguish that concern from presentation outputs such as findings or CNL. Request concrete execution and the operator-selected auxiliary assurances. Preserve the exact task instruction as provenance and do not broaden the request merely to make an inherited circuit selectable.
